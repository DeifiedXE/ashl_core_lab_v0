"""Execute approved-purpose sandbox direct_command records once inside sandbox scope."""
...
COMMAND = "run-approved-purpose-sandbox-direct-command-execution-minimal-check"
FLOW = "approved_purpose_sandbox_direct_command_execution_minimal_v0"
PACKAGE_ID = "PKG-Phase0-ApprovedPurposeSandboxDirectCommandExecution-Minimal-v0"
BOUNDARY_INDEX_BEFORE = "2026-06-09-b131"
BOUNDARY_INDEX_AFTER = "2026-06-09-b132"
...
COMMAND_EXECUTION_RESULTS = {
    ...
    "sandbox.approved_purpose.offer_low_pressure_support": {
        "operation": "offer_low_pressure_support",
        "execution_result": "sandbox_command_dispatched_offer_low_pressure_support",
        "result_interpretation": "direct_command_dispatched_inside_sandbox_without_outcome_observation",
    },
}
...
def build_approved_purpose_sandbox_direct_command_execution_record(
    ...
    source = (
        ...
        else build_approved_purpose_sandbox_direct_command_execution_approval_boundary_record()
    )
    source_validation = validate_approved_purpose_sandbox_direct_command_execution_approval_boundary_record(source)
    ...
    return {
        "execution_record_id": f"approved_purpose_sandbox_direct_command_execution_{purpose}_demo_001",
        "record_type": "approved_purpose_sandbox_direct_command_execution_minimal",
        "record_version": "v0",
        ...
        "sandbox_execution": {
            ...
            "execution_source": "approved_purpose_sandbox_direct_command_execution_approval_boundary",
        ...
        "human_summary": {
            "what_was_executed": f"Approved-purpose sandbox direct_command {direct_command} executed once.",
            "what_was_recorded": "A sandbox-only execution record and dispatch result were recorded.",
            "what_is_blocked": "Outcome observation, feedback, persistence, predictor access or mutation, manipulation, and proof claims remain blocked.",
            "plain_result": "The approved-purpose command was executed once in sandbox, but its outcome has not been observed yet.",
        },
        "blocked_flags": {field: False for field in BLOCKED_FLAGS},
    }


def validate_approved_purpose_sandbox_direct_command_execution_record(
    record: dict[str, Any],
) -> dict[str, Any]:
    errors: list[str] = []
    ...
    return {
        ...
        "feedback_loop_blocked": blocked.get("feedback_loop_created") is False,
...
def run_approved_purpose_sandbox_direct_command_execution_minimal_check() -> dict[str, Any]:
    source_records = run_approved_purpose_sandbox_direct_command_execution_approval_boundary_minimal_check()[
        "valid_records"
    ]
    valid_records = [
        build_approved_purpose_sandbox_direct_command_execution_record(source)
        for source in source_records
    ]
    records = [*valid_records, *_invalid_records(valid_records[0], valid_records[1], valid_records[2])]
    validation_results = [
        validate_approved_purpose_sandbox_direct_command_execution_record(record)
    ...
    return {
        ...
        "boundary": {
            ...
            "boundary_change_required": True,
            "boundary_reason": "Executes approved-purpose sandbox direct_command records once inside sandbox scope.",
        },
        "valid_records": valid_records,
        "validation_results": validation_results,
        "summary": summary,
        "human_summary": {
            "what_was_built": "Approved-purpose sandbox direct_command execution was added.",
            "what_changed": "Approved-purpose direct_commands can now execute once in sandbox-only scope.",
            "what_is_blocked": "Outcome observation, feedback, persistence, predictor mutation, manipulation, and proof claims remain blocked.",
            "plain_result": "Purpose can now drive one sandbox-only command execution, but outcome review is still a future step.",
        },
...
def _invalid_records(reward: dict[str, Any], mismatch: dict[str, Any], comfort: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    mutate(reward, "bad_record_type", ("record_type",), "approved_purpose_execution")
    mutate(reward, "wrong_boundary_after", ("boundary_index_after",), BOUNDARY_INDEX_BEFORE)
    mutate(reward, "source_not_validated", ("source_execution_approval_boundary", "source_validated"), False)
    mutate(reward, "source_future_not_allowed", ("source_execution_approval_boundary", "future_execution_allowed"), False)
    mutate(reward, "source_wrong_scope", ("source_execution_approval_boundary", "execution_scope"), "production")
    mutate(reward, "source_already_executed", ("source_execution_approval_boundary", "source_sandbox_action_executed_in_source_package"), True)
    mutate(reward, "execution_not_created", ("sandbox_execution", "execution_created"), False)
    mutate(reward, "wrong_execution_scope", ("sandbox_execution", "execution_scope"), "production")
    mutate(reward, "wrong_direct_command", ("sandbox_execution", "direct_command"), "sandbox.approved_purpose.wait")
    mutate(reward, "not_executed", ("sandbox_execution", "direct_command_executed"), False)
    mutate(reward, "execution_count_too_high", ("sandbox_execution", "execution_count"), 2)
    mutate(reward, "budget_not_one", ("sandbox_execution", "execution_budget"), 2)
    mutate(reward, "result_not_created", ("sandbox_execution", "execution_result_created"), False)
    mutate(reward, "wrong_result", ("sandbox_execution", "execution_result"), "free_text_result")
    mutate(reward, "outcome_observed", ("sandbox_execution", "outcome_observation_created"), True)
    mutate(reward, "future_outcome_boundary_missing", ("sandbox_execution", "future_outcome_observation_requires_separate_boundary"), False)
    mutate(reward, "wrong_payload_operation", ("sandbox_execution", "command_payload", "operation"), "retry_same_action")
    mutate(reward, "rollback_dirty", ("rollback_preview", "dirty_state_after_rollback"), True)
    mutate(mismatch, "feedback_loop", ("blocked_flags", "feedback_loop_created"), True)
...
def _all_checks_passed(summary: dict[str, int]) -> bool:
    return (
        ...
        and summary["invalid_execution_count"] == 31
        ...
        and summary["approach_or_reach_item_execution_count"] == 1
        and summary["resolve_mismatch_execution_count"] == 1
        and summary["support_user_comfort_execution_count"] == 1
