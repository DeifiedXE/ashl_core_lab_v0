"""Approval boundary from arbitration direct_command to future execution."""
...
COMMAND = "run-sandbox-candidate-ordering-arbitration-direct-command-execution-approval-boundary-minimal-check"
FLOW = "sandbox_candidate_ordering_arbitration_direct_command_execution_approval_boundary_minimal_v0"
PACKAGE_ID = "PKG-Phase0-SandboxCandidateOrderingArbitrationDirectCommandExecutionApprovalBoundary-Minimal-v0"
BOUNDARY_INDEX_BEFORE = "2026-06-09-b145"
BOUNDARY_INDEX_AFTER = "2026-06-09-b146"
...
def build_sandbox_candidate_ordering_arbitration_direct_command_execution_approval_boundary_record(
    ...
    source = (
        ...
        else build_sandbox_candidate_ordering_arbitration_direct_command_record()
    )
    source_validation = validate_sandbox_candidate_ordering_arbitration_direct_command_record(source)
    ...
    return {
        "execution_approval_boundary_id": (
            f"sandbox_candidate_ordering_arbitration_direct_command_execution_approval_boundary_{scenario}_demo_001"
        ),
        "record_type": "sandbox_candidate_ordering_arbitration_direct_command_execution_approval_boundary_minimal",
        "record_version": "v0",
        ...
        "execution_approval_boundary": {
            ...
            "allowed_next_package": "Sandbox Candidate Ordering Arbitration Direct Command Execution Minimal v0",
            "candidate_for_future_execution": direct_command,
            "candidate_source": "sandbox_candidate_ordering_arbitration_direct_command",
        ...
        "human_summary": {
            ...
            "what_it_allows": "A future package may execute this sandbox-only arbitration direct_command once.",
            "what_is_blocked": "This package executes nothing and creates no execution result, persistence, predictor access or mutation, direct feedback feeds, production behavior, or proof claims.",
            "plain_result": "The arbitration direct command can approach execution, but it is not executed yet.",
        },
        "blocked_flags": {field: False for field in BLOCKED_FLAGS},
    }


def validate_sandbox_candidate_ordering_arbitration_direct_command_execution_approval_boundary_record(
    record: dict[str, Any],
) -> dict[str, Any]:
    errors: list[str] = []
...
def run_sandbox_candidate_ordering_arbitration_direct_command_execution_approval_boundary_minimal_check() -> dict[str, Any]:
    source_records = run_sandbox_candidate_ordering_arbitration_direct_command_minimal_check()["valid_records"]
    valid_records = [
        build_sandbox_candidate_ordering_arbitration_direct_command_execution_approval_boundary_record(source)
        for source in source_records
    ]
    records = [*valid_records, *_invalid_records(valid_records[0], valid_records[1], valid_records[2])]
    validation_results = [
        validate_sandbox_candidate_ordering_arbitration_direct_command_execution_approval_boundary_record(record)
    ...
    return {
        ...
        "boundary": {
            ...
            "boundary_change_required": True,
            "boundary_reason": "Opens an approval boundary for future sandbox-only execution from arbitration direct_command.",
        },
        "valid_records": valid_records,
        "validation_results": validation_results,
        "summary": summary,
        "human_summary": {
            "what_was_built": "Arbitration direct_command can now reach a future sandbox execution approval boundary.",
            "what_changed": "Sandbox direct_command records may become future execution candidates.",
            "what_is_blocked": "No execution, execution result, persistence, predictor use or mutation, direct feedback feeds, production behavior, or proof claims are created.",
            "plain_result": "The arbitration path has a checked gate before execution.",
        },
...
def _source_summary(source: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_selected_action": command["source_selected_action"],
        ...
        "source_execution_result_created": command["execution_result_created"],
...
def _validate_source(source: dict[str, Any], errors: list[str]) -> None:
    ...
    if source.get("direct_command_source") != "sandbox_candidate_ordering_arbitration_direct_command_approval_boundary":
    ...
    for field in (
        ...
        "source_execution_result_created",
...
def _invalid_records(first: dict[str, Any], second: dict[str, Any], third: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    mutate(first, "bad_record_type", ("record_type",), "arbitration_execution_boundary")
    ...
    mutate(first, "source_wrong_command_source", ("source_sandbox_direct_command", "direct_command_source"), "wrong_source")
    mutate(first, "source_executed", ("source_sandbox_direct_command", "source_sandbox_action_executed"), True)
    mutate(first, "source_result_created", ("source_sandbox_direct_command", "source_execution_result_created"), True)
    ...
    mutate(first, "wrong_future_execution", ("execution_approval_boundary", "candidate_for_future_execution"), "sandbox.arbitration.wait")
    mutate(first, "wrong_scope", ("execution_approval_boundary", "execution_scope"), "production")
    mutate(first, "executed", ("execution_approval_boundary", "sandbox_action_executed_in_this_package"), True)
    mutate(first, "result_created", ("execution_approval_boundary", "execution_result_created_in_this_package"), True)
    mutate(first, "future_observation_missing", ("execution_approval_boundary", "future_outcome_observation_requires_separate_boundary"), False)
...
def _all_checks_passed(summary: dict[str, int]) -> bool:
    return (
        summary["execution_approval_boundary_result_count"] == 30
        and summary["valid_execution_approval_boundary_count"] == 3
        and summary["invalid_execution_approval_boundary_count"] == 27
