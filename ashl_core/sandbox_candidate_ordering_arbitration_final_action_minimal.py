"""Create sandbox-only final_action from arbitration final-action approval boundary."""
...
COMMAND = "run-sandbox-candidate-ordering-arbitration-final-action-minimal-check"
FLOW = "sandbox_candidate_ordering_arbitration_final_action_minimal_v0"
PACKAGE_ID = "PKG-Phase0-SandboxCandidateOrderingArbitrationFinalAction-Minimal-v0"
BOUNDARY_INDEX_BEFORE = "2026-06-09-b142"
BOUNDARY_INDEX_AFTER = "2026-06-09-b143"
...
def build_sandbox_candidate_ordering_arbitration_final_action_record(
    ...
    source = (
        ...
        else build_sandbox_candidate_ordering_arbitration_final_action_approval_boundary_record()
    )
    validation = validate_sandbox_candidate_ordering_arbitration_final_action_approval_boundary_record(source)
    ...
    return {
        "final_action_record_id": f"sandbox_candidate_ordering_arbitration_final_action_{scenario}_demo_001",
        "record_type": "sandbox_candidate_ordering_arbitration_final_action_minimal",
        "record_version": "v0",
        ...
        "sandbox_final_action": {
            ...
            "final_action_source": "sandbox_candidate_ordering_arbitration_final_action_approval_boundary",
            "final_action_scope": "sandbox_only",
            "final_action_reason": "arbitration_checked_selected_action_final_action_boundary",
        ...
        "human_summary": {
            "what_was_finalized": f"Arbitration selected_action created sandbox-only final_action {final_action}.",
            "what_changed": "A sandbox selected_action became a sandbox-only final_action record.",
            "what_is_blocked": "Direct command, execution, persistence, memory write, predictor use, direct feed, production behavior, and proof claims remain blocked.",
            "plain_result": "Arbitration ordering can now prepare a sandbox final_action, but it still cannot command or execute it.",
        },
        "blocked_flags": {field: False for field in BLOCKED_FLAGS},
    }


def validate_sandbox_candidate_ordering_arbitration_final_action_record(record: dict[str, Any]) -> dict[str, Any]:
    errors: list[str] = []
...
def run_sandbox_candidate_ordering_arbitration_final_action_minimal_check() -> dict[str, Any]:
    source_records = run_sandbox_candidate_ordering_arbitration_final_action_approval_boundary_minimal_check()[
        "valid_records"
    ]
    valid_records = [build_sandbox_candidate_ordering_arbitration_final_action_record(source) for source in source_records]
    records = [*valid_records, *_invalid_records(valid_records[0], valid_records[1], valid_records[2])]
    validation_results = [validate_sandbox_candidate_ordering_arbitration_final_action_record(record) for record in records]
    ...
    return {
        ...
        "boundary": {
            ...
            "boundary_change_required": True,
            "boundary_reason": "Creates sandbox-only final_action records from arbitration final-action approval boundary.",
        },
        "valid_records": valid_records,
        "validation_results": validation_results,
        "summary": summary,
        "human_summary": {
            "what_was_built": "Arbitration sandbox final_action creation was added.",
            "what_changed": "Arbitration selected_actions can become sandbox-only final_action records.",
            "what_is_blocked": "Direct command, execution, persistence, predictor use, direct feed, production behavior, and proof claims remain blocked.",
            "plain_result": "Arbitration ordering can now prepare a sandbox final_action, but it still cannot command or execute it.",
        },
...
def _source_summary(source: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "scenario_id": source_selected["scenario_id"],
        ...
        "source_arbitration_rules_preserved": source_selected["source_arbitration_rules_preserved"],
    }
...
def _invalid_records(first: dict[str, Any], second: dict[str, Any], third: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    mutate(first, "source_rules_not_preserved", ("source_final_action_approval_boundary", "source_arbitration_rules_preserved"), False)
    ...
    mutate(first, "wrong_scope", ("sandbox_final_action", "final_action_scope"), "production")
    mutate(first, "wrong_source", ("sandbox_final_action", "final_action_source"), "unapproved_candidate")
    mutate(first, "rules_not_preserved", ("sandbox_final_action", "arbitration_rules_preserved"), False)
    ...
    mutate(first, "future_direct_boundary_missing", ("sandbox_final_action", "future_direct_command_requires_separate_boundary"), False)
...
def _all_checks_passed(summary: dict[str, int]) -> bool:
    return (
        summary["final_action_result_count"] == 30
