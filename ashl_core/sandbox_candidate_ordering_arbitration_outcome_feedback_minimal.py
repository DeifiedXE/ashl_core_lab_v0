"""Create same-session sandbox feedback records from arbitration outcomes."""
...
COMMAND = "run-sandbox-candidate-ordering-arbitration-outcome-feedback-minimal-check"
FLOW = "sandbox_candidate_ordering_arbitration_outcome_feedback_minimal_v0"
PACKAGE_ID = "PKG-Phase0-SandboxCandidateOrderingArbitrationOutcomeFeedback-Minimal-v0"
BOUNDARY_INDEX_BEFORE = "2026-06-09-b149"
BOUNDARY_INDEX_AFTER = "2026-06-09-b150"

FEEDBACK_SHAPES = {
    "arbitration_positive_item_contact_feedback": {
        "feedback_label": "arbitration_positive_item_contact_feedback_recorded",
        "feedback_valence": "bounded_positive",
        "success_signal": 1.0,
        "blocked_signal": 0.0,
        "uncertainty_signal": 0.0,
        "harm_signal": 0.0,
        "interpretation": "The arbitration sandbox command reached the front item.",
    },
    "arbitration_wait_context_observation_feedback": {
        "feedback_label": "arbitration_wait_context_feedback_recorded",
        "feedback_valence": "bounded_context_observation",
        "success_signal": 0.2,
        "blocked_signal": 0.0,
        "uncertainty_signal": 0.4,
        "harm_signal": 0.0,
        "interpretation": "The arbitration sandbox command waited or observed local context.",
    },
    "arbitration_mismatch_probe_context_feedback": {
        "feedback_label": "arbitration_mismatch_probe_context_feedback_recorded",
        "feedback_valence": "bounded_probe_resolution",
        "success_signal": 0.4,
        "blocked_signal": 0.0,
        "uncertainty_signal": 0.2,
        "harm_signal": 0.0,
        "interpretation": "The arbitration sandbox command probed context for mismatch resolution.",
    },
}
...
def build_sandbox_candidate_ordering_arbitration_outcome_feedback_record(
    ...
    source = (
        ...
        else build_sandbox_candidate_ordering_arbitration_outcome_feedback_approval_boundary_record()
    )
    source_validation = validate_sandbox_candidate_ordering_arbitration_outcome_feedback_approval_boundary_record(
        source
    )
    ...
    return {
        "feedback_record_id": f"sandbox_candidate_ordering_arbitration_outcome_feedback_{scenario}_demo_001",
        "record_type": "sandbox_candidate_ordering_arbitration_outcome_feedback_minimal",
        "record_version": "v0",
        ...
        "same_session_feedback": {
            ...
            "feedback_source": "sandbox_candidate_ordering_arbitration_outcome_feedback_approval_boundary",
        ...
        "human_summary": {
            "what_was_created": f"Same-session sandbox feedback record {feedback_type} was created.",
            "what_it_means": shape["interpretation"],
            "what_is_blocked": "The feedback is not applied, does not reorder candidates, creates no action or execution, writes no persistence, touches no predictor, and makes no proof claim.",
            "plain_result": "The observed arbitration outcome now has bounded feedback evidence, but it still cannot change the next choice.",
        },
        "blocked_flags": {field: False for field in BLOCKED_FLAGS},
    }


def validate_sandbox_candidate_ordering_arbitration_outcome_feedback_record(
    record: dict[str, Any],
) -> dict[str, Any]:
    errors: list[str] = []
...
def run_sandbox_candidate_ordering_arbitration_outcome_feedback_minimal_check() -> dict[str, Any]:
    source_records = run_sandbox_candidate_ordering_arbitration_outcome_feedback_approval_boundary_minimal_check()[
        "valid_records"
    ]
    valid_records = [
        build_sandbox_candidate_ordering_arbitration_outcome_feedback_record(source)
        for source in source_records
    ]
    records = [*valid_records, *_invalid_records(valid_records[0], valid_records[1], valid_records[2])]
    validation_results = [
        validate_sandbox_candidate_ordering_arbitration_outcome_feedback_record(record)
    ...
    return {
        ...
        "boundary": {
            ...
            "boundary_change_required": True,
            "boundary_reason": "Creates same-session sandbox feedback records from arbitration outcome approval boundaries.",
        },
        "valid_records": valid_records,
        "validation_results": validation_results,
        "summary": summary,
        "human_summary": {
            "what_was_built": "Arbitration sandbox outcome feedback records can now be created for the same session.",
            "what_changed": "Outcome observations can become bounded sandbox-only feedback evidence.",
            "what_is_blocked": "Feedback is not applied, does not reorder candidates, creates no action or execution, writes no memory, touches no predictor, and makes no proof claim.",
            "plain_result": "The arbitration line can now record feedback from observed outcomes, but the feedback cannot change behavior yet.",
        },
...
def _source_summary(source: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "feedback_evaluation_created_in_source_package": boundary["feedback_evaluation_created_in_this_package"],
...
def _invalid_records(reach: dict[str, Any], wait: dict[str, Any], probe: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    mutate(reach, "bad_record_type", ("record_type",), "sandbox_arbitration_feedback_runtime")
    ...
    mutate(reach, "source_wrong_boundary_index", ("source_feedback_approval_boundary", "source_boundary_index"), "2026-06-09-b148")
    ...
    mutate(reach, "source_bad_feedback_candidate", ("source_feedback_approval_boundary", "candidate_for_future_feedback"), "unknown")
    mutate(reach, "source_feedback_evaluation_already_created", ("source_feedback_approval_boundary", "feedback_evaluation_created_in_source_package"), True)
    mutate(reach, "source_feedback_applied", ("source_feedback_approval_boundary", "feedback_applied_in_source_package"), True)
    mutate(reach, "source_feedback_loop_created", ("source_feedback_approval_boundary", "feedback_loop_created_in_source_package"), True)
    mutate(reach, "source_reordering_created", ("source_feedback_approval_boundary", "candidate_reordering_created_in_source_package"), True)
    ...
    mutate(wait, "future_reordering_boundary_missing", ("same_session_feedback", "future_candidate_reordering_requires_separate_boundary"), False)
...
def _all_checks_passed(summary: dict[str, int]) -> bool:
    return (
        summary["feedback_result_count"] == 42
        and summary["valid_feedback_count"] == 3
        and summary["invalid_feedback_count"] == 39
