"""Approval boundary for future reordered-candidate feedback-gated reordering."""
...
COMMAND = (
    "run-sandbox-candidate-ordering-arbitration-reordered-candidate-feedback-gated-candidate-"
    "reordering-approval-boundary-minimal-check"
)
FLOW = "sandbox_candidate_ordering_arbitration_reordered_candidate_feedback_gated_candidate_reordering_approval_boundary_minimal_v0"
PACKAGE_ID = "PKG-Phase0-SandboxCandidateOrderingArbitrationReorderedCandidateFeedbackGatedCandidateReorderingApprovalBoundary-Minimal-v0"
BOUNDARY_INDEX_BEFORE = "2026-06-09-b167"
BOUNDARY_INDEX_AFTER = "2026-06-09-b168"
...
FEEDBACK_APPLICATION_TO_REORDERING_TARGET = {
    "arbitration_reordered_positive_item_contact_feedback_application": {
        ...
        "future_reordering_label": "reordered_positive_item_contact_feedback_may_prioritize_reach",
        "boundary_reason": "Record-only reordered positive item contact feedback may support future advisory reordering.",
    },
    "arbitration_reordered_wait_context_observation_feedback_application": {
        "candidate_for_future_reordering": "wait_or_observe",
        "future_reordering_label": "reordered_wait_context_feedback_may_prioritize_observation",
        "boundary_reason": "Record-only reordered wait/context feedback may support future advisory observation ordering.",
    },
    "arbitration_reordered_mismatch_probe_context_feedback_application": {
        ...
        "future_reordering_label": "reordered_mismatch_probe_feedback_may_prioritize_probe",
        "boundary_reason": "Record-only reordered mismatch probe feedback may support future advisory probe ordering.",
    },
}
...
def build_sandbox_candidate_ordering_arbitration_reordered_candidate_feedback_gated_candidate_reordering_approval_boundary_record(
    ...
    source = (
        ...
        else build_sandbox_candidate_ordering_arbitration_reordered_candidate_outcome_feedback_application_record()
    )
    source_validation = (
        validate_sandbox_candidate_ordering_arbitration_reordered_candidate_outcome_feedback_application_record(source)
    )
    ...
    return {
        "reordering_approval_boundary_id": (
            "sandbox_candidate_ordering_arbitration_reordered_candidate_feedback_gated_"
            f"reordering_approval_boundary_{scenario}_demo_001"
        ),
        "record_type": (
            "sandbox_candidate_ordering_arbitration_reordered_candidate_feedback_gated_candidate_reordering_"
        ...
        "feedback_gated_reordering_approval_boundary": {
            ...
            "allowed_next_package": (
                "Sandbox Candidate Ordering Arbitration Reordered Candidate Feedback-Gated Candidate "
                "Reordering Minimal v0"
            ),
            ...
            "candidate_source": "sandbox_candidate_ordering_arbitration_reordered_candidate_outcome_feedback_application",
        ...
        "human_summary": {
            "what_was_opened": (
                "A future sandbox-only reordered-candidate feedback-gated reordering approval boundary was opened."
            ),
            "what_it_allows": f"A future package may consider {application_type} for advisory candidate reordering.",
            "what_is_blocked": (
                "This package does not reorder candidates, change scores, alter next-cycle ordering, create actions, "
                "execute, observe new outcomes, persist feedback, write memory, touch predictors, feed endocrine/"
                "tendency systems, change production behavior, or prove learning."
            ),
            "plain_result": (
                "The reordered feedback can be reviewed for a future candidate-list change, but no order changed now."
            ),
        },
...
def validate_sandbox_candidate_ordering_arbitration_reordered_candidate_feedback_gated_candidate_reordering_approval_boundary_record(
    record: dict[str, Any],
) -> dict[str, Any]:
    errors: list[str] = []
...
def run_sandbox_candidate_ordering_arbitration_reordered_candidate_feedback_gated_candidate_reordering_approval_boundary_minimal_check() -> dict[
    str, Any
]:
    source_records = (
        run_sandbox_candidate_ordering_arbitration_reordered_candidate_outcome_feedback_application_minimal_check()[
    ...
    return {
        ...
        "boundary": {
            ...
            "boundary_change_required": True,
            "boundary_reason": (
                "Opens a future candidate reordering approval boundary from record-only reordered-candidate "
        ...
        "human_summary": {
            "what_was_built": (
                "Record-only reordered-candidate arbitration feedback applications can now reach a future "
                "reordering approval boundary."
            ),
            "what_changed": "The next package may study sandbox-only advisory candidate reordering from b167 records.",
            "what_is_blocked": (
                "No reordering, scoring change, next-cycle ordering change, feedback loop, action, execution, "
                "outcome observation, persistence, predictor influence, production behavior, or proof claim is created."
            ),
            "plain_result": "The reordered feedback can be reviewed for future ordering, but it still cannot change the next choice.",
        },
...
def _source_summary(source: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "selected_action": application["selected_action"],
        "final_action": application["final_action"],
        ...
        "future_memory_write_requires_separate_boundary": application["future_memory_write_requires_separate_boundary"],
        "future_retention_requires_separate_boundary": application["future_retention_requires_separate_boundary"],
        "future_predictor_influence_requires_separate_boundary": application[
            "future_predictor_influence_requires_separate_boundary"
        ],
        "future_production_promotion_requires_separate_boundary": application[
...
def _invalid_records(reach: dict[str, Any], wait: dict[str, Any], probe: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    mutate(reach, "bad_record_type", ("record_type",), "sandbox_reordered_reordering_runtime")
    ...
    mutate(reach, "source_wrong_boundary_index", ("source_feedback_application", "source_boundary_index"), "2026-06-09-b166")
    ...
    mutate(
        reach,
        "wrong_next_package",
        ("feedback_gated_reordering_approval_boundary", "allowed_next_package"),
        "Sandbox Candidate Ordering Arbitration Reordered Candidate Feedback-Gated Candidate Reordering Minimal v1",
    )
...
def _summary(validation_results: list[dict[str, Any]]) -> dict[str, int]:
    ...
    return {
        ...
        "positive_item_reordering_boundary_count": sum(
            1 for result in valid if result["candidate_for_future_reordering"] == "reach_front_item"
        ),
        "wait_context_reordering_boundary_count": sum(
            1 for result in valid if result["candidate_for_future_reordering"] == "wait_or_observe"
        ),
        "mismatch_probe_reordering_boundary_count": sum(
            1 for result in valid if result["candidate_for_future_reordering"] == "observe_or_alternative_probe"
        ),
