"""Approval boundary from reordered-candidate feedback to future application."""
...
COMMAND = (
    "run-sandbox-candidate-ordering-arbitration-reordered-candidate-outcome-feedback-application-"
    "approval-boundary-minimal-check"
)
FLOW = "sandbox_candidate_ordering_arbitration_reordered_candidate_outcome_feedback_application_approval_boundary_minimal_v0"
PACKAGE_ID = (
    "PKG-Phase0-SandboxCandidateOrderingArbitrationReorderedCandidateOutcomeFeedbackApplicationApprovalBoundary-"
    "Minimal-v0"
)
BOUNDARY_INDEX_BEFORE = "2026-06-09-b165"
BOUNDARY_INDEX_AFTER = "2026-06-09-b166"
...
def build_sandbox_candidate_ordering_arbitration_reordered_candidate_outcome_feedback_application_approval_boundary_record(
    feedback_record: dict[str, Any] | None = None,
) -> dict[str, Any]:
    source = (
        deepcopy(feedback_record)
        if feedback_record is not None
        else build_sandbox_candidate_ordering_arbitration_reordered_candidate_outcome_feedback_record()
    )
    source_validation = validate_sandbox_candidate_ordering_arbitration_reordered_candidate_outcome_feedback_record(
        source
    )
    ...
    return {
        "feedback_application_approval_boundary_id": (
            "sandbox_candidate_ordering_arbitration_reordered_candidate_outcome_feedback_application_approval_"
            f"boundary_{scenario}_demo_001"
        ),
        "record_type": (
            "sandbox_candidate_ordering_arbitration_reordered_candidate_outcome_feedback_application_approval_"
            "boundary_minimal"
        ...
        "feedback_application_approval_boundary": {
            ...
            "candidate_source": "sandbox_candidate_ordering_arbitration_reordered_candidate_outcome_feedback",
        ...
        "human_summary": {
            "what_was_opened": (
                f"Reordered feedback type {feedback_type} may enter a future same-session sandbox "
                "application package."
            ),
            "what_it_allows": (
                "A future package may decide whether this reordered-candidate feedback can be applied "
                "inside the same-session sandbox."
            ),
            "what_is_blocked": (
                "This package applies no feedback, creates no loop, reorders no candidates, changes no "
                "score, creates no action or execution, writes no persistence, touches no predictor, "
                "and makes no proof claim."
            ),
            "plain_result": (
                "The reordered feedback can approach a future application gate, but it still cannot "
...
def validate_sandbox_candidate_ordering_arbitration_reordered_candidate_outcome_feedback_application_approval_boundary_record(
    record: dict[str, Any],
) -> dict[str, Any]:
    errors: list[str] = []
...
def run_sandbox_candidate_ordering_arbitration_reordered_candidate_outcome_feedback_application_approval_boundary_minimal_check() -> dict[
    str, Any
]:
    source_records = (
        run_sandbox_candidate_ordering_arbitration_reordered_candidate_outcome_feedback_minimal_check()[
    ...
    return {
        ...
        "boundary": {
            ...
            "boundary_change_required": True,
            "boundary_reason": (
                "Opens a future same-session sandbox feedback application boundary from reordered-candidate "
                "feedback records."
            ),
        },
        ...
        "human_summary": {
            "what_was_built": (
                "Reordered-candidate sandbox feedback records can now reach a future application approval boundary."
            ),
            "what_changed": (
                "Same-session sandbox feedback from reordered candidates may be considered by a future "
                "feedback application package."
            ),
            "what_is_blocked": (
                "Feedback is not applied, no loop or reordering is created, no action or execution is "
                "created, no memory is written, no predictor is touched, and no proof claim is made."
            ),
            "plain_result": "The reordered feedback has a checked gate before it can be applied later.",
        },
...
def _source_summary(source: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "new_action_created": feedback["new_action_created"],
        "new_selected_action_created": feedback["new_selected_action_created"],
        "new_final_action_created": feedback["new_final_action_created"],
        "new_direct_command_created": feedback["new_direct_command_created"],
        "new_execution_created": feedback["new_execution_created"],
        "new_outcome_observation_created": feedback["new_outcome_observation_created"],
        ...
        "future_feedback_application_requires_separate_boundary": feedback[
...
def _invalid_records(reach: dict[str, Any], wait: dict[str, Any], probe: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    mutate(reach, "bad_record_type", ("record_type",), "sandbox_reordered_feedback_application")
    ...
    mutate(reach, "source_wrong_boundary_index", ("source_feedback_record", "source_boundary_index"), "2026-06-09-b164")
    ...
    mutate(reach, "source_new_action_created", ("source_feedback_record", "new_action_created"), True)
    mutate(reach, "source_new_selected_action_created", ("source_feedback_record", "new_selected_action_created"), True)
    mutate(reach, "source_new_final_action_created", ("source_feedback_record", "new_final_action_created"), True)
    mutate(reach, "source_new_direct_command_created", ("source_feedback_record", "new_direct_command_created"), True)
    mutate(reach, "source_new_execution_created", ("source_feedback_record", "new_execution_created"), True)
    mutate(reach, "source_new_outcome_observation_created", ("source_feedback_record", "new_outcome_observation_created"), True)
    mutate(
        reach,
        "source_future_application_boundary_missing",
        ("source_feedback_record", "future_feedback_application_requires_separate_boundary"),
        False,
    )
    mutate(reach, "source_bad_feedback_type", ("source_feedback_record", "feedback_type"), "unknown")
    mutate(
        reach,
        "future_application_not_allowed",
        ("feedback_application_approval_boundary", "future_feedback_application_allowed"),
        False,
    )
    mutate(
        reach,
        "wrong_next_package",
        ("feedback_application_approval_boundary", "allowed_next_package"),
        "Feedback Application v0",
    )
    mutate(
        reach,
        "wrong_application_candidate",
        ("feedback_application_approval_boundary", "candidate_for_future_feedback_application"),
        "unknown",
    )
    mutate(
        reach,
        "wrong_candidate_source",
        ("feedback_application_approval_boundary", "candidate_source"),
        "sandbox_candidate_ordering_arbitration_outcome_feedback",
    )
    mutate(
        reach,
        "wrong_application_scope",
        ("feedback_application_approval_boundary", "feedback_application_scope"),
        "production",
    )
    mutate(
        reach,
        "feedback_applied",
        ("feedback_application_approval_boundary", "feedback_applied_in_this_package"),
        True,
    )
    mutate(
        reach,
        "feedback_loop_created",
        ("feedback_application_approval_boundary", "feedback_loop_created_in_this_package"),
        True,
    )
    mutate(
        reach,
        "candidate_reordering_created",
        ("feedback_application_approval_boundary", "candidate_reordering_created_in_this_package"),
        True,
    )
    mutate(
        reach,
        "candidate_scores_changed",
        ("feedback_application_approval_boundary", "candidate_scores_changed_in_this_package"),
        True,
    )
    mutate(
        reach,
        "next_cycle_ordering_changed",
        ("feedback_application_approval_boundary", "next_cycle_candidate_ordering_changed_in_this_package"),
        True,
    )
    mutate(
        reach,
        "new_action_created",
        ("feedback_application_approval_boundary", "new_action_created_in_this_package"),
        True,
    )
    mutate(
        reach,
        "new_selected_action_created",
        ("feedback_application_approval_boundary", "new_selected_action_created_in_this_package"),
        True,
    )
    mutate(
        reach,
        "new_final_action_created",
        ("feedback_application_approval_boundary", "new_final_action_created_in_this_package"),
        True,
    )
    mutate(
        reach,
        "new_direct_command_created",
        ("feedback_application_approval_boundary", "new_direct_command_created_in_this_package"),
        True,
    )
    mutate(
        reach,
        "new_execution_created",
        ("feedback_application_approval_boundary", "new_execution_created_in_this_package"),
        True,
    )
    mutate(
        reach,
        "new_outcome_observation_created",
        ("feedback_application_approval_boundary", "new_outcome_observation_created_in_this_package"),
        True,
    )
    mutate(
        wait,
        "future_reordering_boundary_missing",
        ("feedback_application_approval_boundary", "future_candidate_reordering_requires_separate_boundary"),
        False,
    )
...
def _all_checks_passed(summary: dict[str, int]) -> bool:
    return (
        summary["feedback_application_approval_boundary_result_count"] == 51
        and summary["valid_feedback_application_approval_boundary_count"] == 3
        and summary["invalid_feedback_application_approval_boundary_count"] == 48
