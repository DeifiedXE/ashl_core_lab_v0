"""Append-only external-state store for Package 127."""
...
PACKAGE_127_DIRNAME = "package_127_internal_focus_shift_v0"
PACKAGE_127_FILENAME = "package_127.sqlite3"

TABLE_ID_COLUMNS = {
    "internal_focus_authorizations": "authorization_id",
    "internal_focus_candidate_batches": "focus_candidate_batch_id",
    "internal_focus_candidates": "focus_candidate_id",
    "internal_focus_selections": "focus_selection_id",
    "internal_focus_policy_decisions": "policy_decision_id",
    "internal_focus_plans": "focus_plan_id",
    "internal_focus_actions": "internal_action_id",
    "focused_visual_region_views": "focused_region_view_id",
    "internal_focus_context_sidecars": "focus_context_id",
    "internal_focus_release_records": "focus_release_record_id",
    "package_127_score_equivalence_records": "score_equivalence_record_id",
    "package_127_control_results": "control_result_id",
    "package_127_real_run_records": "real_run_record_id",
    "operator_event_delivery_failures": "event_delivery_failure_id",
    "package_127_audits": "audit_id",
}


class Package127InternalFocusStore:
    def __init__(self, state_dir: str | Path) -> None:
        if state_dir is None:
            raise ValueError("explicit external state_dir is required")
        self.state_dir = Path(state_dir)
        self.root_dir = self.state_dir / PACKAGE_127_DIRNAME
        self.db_path = self.root_dir / PACKAGE_127_FILENAME
    ...
    def append_payload(
        ...
        if id_column != self._id_column(table):
            raise ValueError("Package 127 id column mismatch")
    ...
    def get_payload(self, table: str, record_id: str) -> dict[str, Any]:
        ...
        if row is None:
            raise KeyError(f"missing Package 127 record: {record_id}")
    ...
    def _initialize(self) -> None:
        with self.connection() as connection:
            ...
            connection.execute(
                ...
                """,
                (
                    "ashl_package_127_internal_focus_store",
                    "v0",
                    utc_now(),
                ),
            )
    ...
    def _id_column(table: str) -> str:
        try:
            ...
            raise ValueError(f"unknown Package 127 table: {table}") from error
