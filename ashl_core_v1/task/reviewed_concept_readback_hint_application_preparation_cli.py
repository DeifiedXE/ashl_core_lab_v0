"""CLI for TaskWorkingMemoryReadbackHint application preparation demos."""
...
from ashl_core_v1.task.reviewed_concept_readback_hint_application_preparation import (
    ...
    build_demo_blocked_task_working_memory_readback_hint_application_preparation_set,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="ASHL Core v1 TaskWorkingMemoryReadbackHint application preparation CLI"
    )
    subparsers = parser.add_subparsers(dest="command", required=True)
    subparsers.add_parser("prepare-demo-application")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "prepare-demo-application":
            return _print_json(
                build_demo_task_working_memory_readback_hint_application_preparation_set()
            )
        if args.command == "show-demo-preparation":
            ...
            return _print_json(payload["hint_application_preparation_set"])
        if args.command == "show-demo-safety-audit":
            payload = (
                build_demo_task_working_memory_readback_hint_application_preparation_set()
            )
            return _print_json(payload["hint_application_preparation_safety_audit"])
        ...
        if args.command == "prepare-demo-held":
            if args.case != "all-held":
                raise ValueError(
                    f"unknown held application preparation case: {args.case}"
                )
            return _print_json(
                build_demo_all_held_task_working_memory_readback_hint_application_preparation_set()
            )
