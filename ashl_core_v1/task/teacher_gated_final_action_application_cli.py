"""CLI for teacher-gated final_action application demos."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="ASHL Core v1 teacher-gated final_action application CLI"
    )
    subparsers = parser.add_subparsers(dest="command", required=True)
    subparsers.add_parser("apply-demo-final-action")
    ...
    subparsers.add_parser("rollback-demo-final-action")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "apply-demo-final-action":
            return _print_json(build_demo_final_action_application())
        if args.command == "show-demo-teacher-gate":
            payload = build_demo_final_action_application()
            return _print_json(payload["final_action_application_gate"])
        if args.command == "show-demo-application":
            payload = build_demo_final_action_application()
            return _print_json(payload["final_action_application"])
        if args.command == "show-demo-rollback":
            payload = build_demo_final_action_application()
            return _print_json(payload["final_action_rollback"])
        if args.command == "show-demo-audit":
            payload = build_demo_final_action_application()
            return _print_json(payload["final_action_application_audit"])
        ...
        if args.command == "rollback-demo-final-action":
            payload = build_demo_final_action_application()
            return _print_json(
                apply_final_action_rollback(payload["final_action_rollback"])
            )
        if args.command == "apply-demo-blocked":
            return _print_json(
                build_demo_blocked_teacher_gated_final_action_application(args.case)
            )
