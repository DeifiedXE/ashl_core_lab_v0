"""Append-only external evidence store for Package 142."""
...
PACKAGE_DIR = "package_142_specialized_thought_bounded_rules_v0"
DATABASE_NAME = "package_142.sqlite3"
STORE_SCHEMA_VERSION = "ashl_package_142_append_only_store_v0"

TABLE_KEYS = {
    "specialized_thought_consumer_bindings": "consumer_binding_id",
    "specialized_thought_rule_family_contracts": "family_contract_id",
    "specialized_thought_precursor_bindings": "precursor_binding_id",
    "specialized_thought_rule_evaluations": "specialized_evaluation_id",
    "bounded_specialized_thought_results": "specialized_result_id",
    "specialized_thought_cross_family_conflicts": "conflict_id",
    "specialized_thought_cascade_invalidations": "invalidation_id",
    "specialized_thought_counterfactual_equivalence_records": "counterfactual_id",
    "package_142_control_results": "control_result_id",
    "package_142_regression_receipts": "regression_receipt_id",
    "package_142_audits": "audit_id",
}

_WINDOWS_ABSOLUTE_PATH = re.compile(r"^[A-Za-z]:[\\/]")


class Package142SpecializedThoughtStore:
    ...
    def append_group(self, records: tuple[tuple[str, Any], ...]) -> None:
        if not records:
            raise ValueError("Package 142 append group cannot be empty")
            ...
            for table, record_id, created_at, serialized, digest in prepared:
                ...
                if existing is not None:
                    ...
                    if not _same_identity_payload(persisted, incoming):
                        raise ValueError(f"Package 142 identity collision: {record_id}")
                    continue
                connection.execute(
                    f"INSERT INTO {table} "
    ...
    def audit_integrity(self) -> dict[str, Any]:
        ...
        with self.connection() as connection:
            ...
            metadata = connection.execute(
                "SELECT schema_version FROM package_142_store_metadata WHERE schema_name = ?",
                (PACKAGE_DIR,),
            ).fetchone()
    ...
    def update(self, *_args: object, **_kwargs: object) -> None:
        raise TypeError("Package 142 evidence is append-only")
    ...
    def _validate_payload(payload: dict[str, Any]) -> None:
        if _contains_private_absolute_path(payload):
            raise ValueError("Package 142 records cannot persist private absolute paths")

    @staticmethod
    def _require_table(table: str) -> None:
        if table not in TABLE_KEYS:
            raise ValueError(f"unsupported Package 142 table: {table}")

    def _initialize(self) -> None:
        with self.connection() as connection:
            connection.execute(
                """
                CREATE TABLE IF NOT EXISTS package_142_store_metadata (
            ...
            connection.execute(
                """
                INSERT OR IGNORE INTO package_142_store_metadata (
...
def package_142_store_path(state_dir: str | Path) -> Path:
