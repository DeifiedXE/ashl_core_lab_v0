"""Append-only external evidence store for Package 144."""
...
PACKAGE_DIR = "package_144_deep_thought_deliberation_v0"
DATABASE_NAME = "package_144.sqlite3"
STORE_SCHEMA_VERSION = "ashl_package_144_append_only_evidence_store_v0"

TABLE_KEYS = {
    "deep_thought_workspace_consumer_bindings": "consumer_binding_id",
    "immutable_workspace_snapshot_contracts": "snapshot_contract_id",
    "immutable_coarse_workspace_snapshots": "snapshot_id",
    "deliberation_operation_allowlists": "operation_allowlist_id",
    "deep_thought_deliberation_authorizations": "authorization_id",
    "deep_thought_deliberation_sessions": "deliberation_session_id",
    "deep_thought_deliberation_steps": "deliberation_step_id",
    "bounded_deep_thought_results": "deliberation_result_id",
    "deep_thought_deliberation_terminals": "terminal_record_id",
    "deep_thought_deliberation_cancellations": "cancellation_id",
    "deep_thought_deliberation_invalidations": "invalidation_id",
    "deep_thought_counterfactual_equivalence_records": "counterfactual_id",
    "package_144_control_results": "control_result_id",
    "package_144_regression_receipts": "regression_receipt_id",
    "package_144_audits": "audit_id",
}

_WINDOWS_ABSOLUTE_PATH = re.compile(r"^[A-Za-z]:[\\/]")


class Package144DeepThoughtDeliberationStore:
    ...
    def append_group(self, records: tuple[tuple[str, Any], ...]) -> None:
        if not records:
            raise ValueError("Package 144 append group cannot be empty")
            ...
            for table, record_id, created_at, serialized, digest in prepared:
                ...
                if existing is not None:
                    ...
                    if not _same_identity_payload(persisted, incoming):
                        raise ValueError(f"Package 144 identity collision: {record_id}")
                    continue
                connection.execute(
                    f"INSERT INTO {table} "
    ...
    def audit_integrity(self) -> dict[str, Any]:
        ...
        with self.connection() as connection:
            ...
            metadata = connection.execute(
                "SELECT schema_version FROM package_144_store_metadata WHERE schema_name = ?",
                (PACKAGE_DIR,),
            ).fetchone()
        ...
        forbidden_tables = {
            ...
            "mutable_workspace_snapshots",
            "deliberation_recovery_heads",
            "thought_trace_boundary",
        ...
        return {
            ...
            "active_deliberation_persisted": False,
            "recovery_head_present": False,
            "authority_table_present": bool(authority_tables),
            "failure_reasons": tuple(failures),
        }

    def update(self, *_args: object, **_kwargs: object) -> None:
        raise TypeError("Package 144 evidence is append-only")
    ...
    def _verified_payload(row: sqlite3.Row, table: str) -> dict[str, Any]:
        ...
        if str(row["payload_sha256"]) != sha256_payload(payload):
            raise RuntimeError(f"blocked_corrupt_package_144_payload:{table}")
        return payload

    @staticmethod
    def _validate_payload(payload: dict[str, Any]) -> None:
        if _contains_private_absolute_path(payload):
            raise ValueError("Package 144 records cannot persist private absolute paths")

    @staticmethod
    def _require_table(table: str) -> None:
        if table not in TABLE_KEYS:
            raise ValueError(f"unsupported Package 144 table: {table}")

    def _initialize(self) -> None:
        with self.connection() as connection:
            connection.execute(
                """
                CREATE TABLE IF NOT EXISTS package_144_store_metadata (
            ...
            connection.execute(
                """
                INSERT OR IGNORE INTO package_144_store_metadata (
...
def package_144_store_path(state_dir: str | Path) -> Path:
