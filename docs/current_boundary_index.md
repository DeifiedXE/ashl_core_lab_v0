...
Previous Boundary Index Version: 2026-06-09-b88
Previous Last update log: Verification Candidate Registry + Trace Minimal v0
...
- Sandbox behavior use, doubt-gated ordering, verification candidate registry, and verification planning exist only as advisory sandbox candidate/trace records; doubt_action exists only as trace/checker evidence for expected/actual mismatch. Selected_action, final_action, direct command, verification execution, predictor mutation, production behavior, memory/retention write, persistent rule, and proof claims remain blocked.
...
- ASHL Core can maintain a bounded registry of trace-only verification candidates, validate that doubt-gated ordering references only registered candidates, and build a one-step trace-only verification plan from observe_or_alternative_probe with plan_budget=1 and fallback_stop_and_report. After expected/actual mismatch, trace-only doubt_action records can make low-risk verification and check_before_retry rank before direct retry. These are advisory/planning/trace-only, not verification execution, selected_action, final_action, direct command, persistent rule, memory/retention write, predictor mutation, production behavior, or proof of learning.
...
- Sandbox Behavior Use milestone / Doubt Action Trace milestone / Doubt-Gated Sandbox Candidate Ordering milestone / Verification Candidate Registry milestone / Verification Planning milestone: ASHL Core can use approved memory-influenced tendency and trace-only doubt_action output to rank sandbox-only candidate actions, constrain them by a bounded verification candidate registry, and build a one-step trace-only verification plan. Evidence: registered candidates observe_or_alternative_probe,inspect_device,check_before_retry,retry_limited,fallback_stop_and_report; all_candidates_trace_only=True; all_candidates_execution_blocked=True; free_form_candidates_allowed=False; candidate_actions_after_ordering=observe_or_alternative_probe,check_before_retry,fallback_stop_and_report,retry_same_action_without_check; verification_candidate_ranked_before_direct_retry=True; check_before_retry_ranked_before_direct_retry=True; ordering_is_advisory=True; expected_outcome=box_pushed; actual_outcome=box_blocked; selected_verification_candidate_id=observe_or_alternative_probe; plan_budget=1; fallback_if_probe_fails=fallback_stop_and_report; verification_execution_allowed=False; verification_action_executed=False. Forbidden: no selected_action, final_action, direct command, persistent rule, memory/retention write, predictor read/influence/mutation, production behavior, autonomous learning/action, or proof-of-learning.
## Archive
- Older milestone details are archived in `docs/boundary_index_archive_2026_06.md`.
