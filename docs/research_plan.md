...
Status: completed / controlled-mentor-override-check / no-production-action-selection.

Progress:
- Uses the same deterministic runtime tendency runner for memory_off, memory_on, and memory_on_with_mentor_override.
...
Status: completed / schema-bridge / trial-trace-preview-only.

Progress:
- Bridges accepted generic reviewed lesson dry-run corrections into the existing dry_run_correction_into_trial_trace path.
- Creates dry-run trial trace preview only for accepted dry-run correction.
- Blocks rejected and needs_more_evidence decisions from trial trace preview.
- Reuses the existing trial trace module instead of creating a source-specific trial trace channel or new trial trace implementation.
- Keeps Level 0 flip evidence as supporting anti-bias evidence and Level 1 contrast evidence as the candidate source.
...
Progress:
- Adds `ashl_core/b95_b97_sandbox_action_boundary_audit_minimal.py`.
- Audits the b95 sandbox-only selected_action boundary, b96 one-step sandbox action execution, and b97 execution-result feedback loop.
- Confirms `selected_action=observe_or_alternative_probe`, `action_executed=True`, `execution_count=1`, and `execution_result=local_context_observed`.
- Confirms same-session feedback, same-session-only reordering, rollback_required, rollback_verified, and `dirty_state_after_rollback=False`.
- Keeps final_action, direct command, persistent trust/doubt update, cross-session feedback persistence, memory write, retained JSONL write, retention write, predictor read/influence/mutation, production behavior, autonomous learning/action, and proof claims blocked.

Boundary:
- Boundary Index remains `2026-06-09-b97`.
- Rationale: this audit checks existing b95-b97 sandbox selected_action / execution / feedback behavior only.
- This does not change permission scope, runtime behavior condition, persistence, memory, retention, predictor, final_action, direct command, production, or proof boundary.
...
Status: completed / sandbox-only direct command created / not executed / Boundary Index b100 -> b101.

Progress:
- Adds `ashl_core/sandbox_direct_command_minimal.py`.
- Creates one sandbox-only direct command from the approved b100 direct-command approval boundary and audited b99 sandbox-only final_action.
- Records `direct_command=sandbox.observe_or_alternative_probe`, `direct_command_scope=sandbox_only`, and `direct_command_executed=False`.
- Keeps direct command execution, production behavior, real navigation/UI changes, persistent rule/trust/doubt updates, cross-session feedback persistence, memory write, retained JSONL write, retention write, predictor read/influence/mutation, autonomous learning/action, and proof claims blocked.
...
Status: completed / sandbox-only direct command executed once / no feedback loop / Boundary Index b102 -> b103.

Progress:
- Adds `ashl_core/sandbox_direct_command_execution_minimal.py`.
- Executes the approved b101/b102 sandbox-only direct command `sandbox.observe_or_alternative_probe` exactly once inside `phase0_level3_sandbox_only`.
- Records `execution_result=local_context_observed`, `execution_count=1`, `execution_budget=1`, `budget_remaining=0`, and `stop_condition_met=True`.
- Keeps feedback loop creation, production behavior, real navigation/UI changes, persistent updates, memory write, retained JSONL write, retention write, predictor read/influence/mutation, autonomous learning/action, and proof claims blocked.
...
Status: completed / future final_action approval boundary for body-motor selected_action / Boundary Index b116.

Progress:
- Adds `ashl_core/sandbox_body_motor_final_action_approval_boundary_minimal.py`.
- Adds `py -3 -m ashl_core.teaching_cli run-sandbox-body-motor-final-action-approval-boundary-minimal-check`.
- Reuses `sandbox_motor_intent_to_selected_action_bridge_minimal` as source.
- Allows future final_action package only for body-motor selected_action records.
- Blocks no-selected-action wall cases from future final_action.
...
Status: completed / approved-purpose ordering to future sandbox selected_action approval boundary / Boundary Index b125.

Progress:
- Reuses Approved Purpose Candidate Ordering Minimal v0 as source.
- Allows top-ranked advisory candidates to enter a future sandbox selected_action package.
- Candidate examples: `reach_front_item`, `observe_or_alternative_probe`, and `offer_low_pressure_support`.
- Creates no selected_action in this package.

Boundary:
- Boundary Index updates from `2026-06-09-b124` to `2026-06-09-b125`.
- This package is selected_action approval-boundary only. It does not create selected_action, create final_action, create direct command, execute sandbox action, perform runtime action selection, change runtime behavior, write memory, write retention, read/influence/mutate predictors, manipulate emotion, claim user happiness, or prove learning.
...
## Approved Purpose Sandbox Final Action Minimal v0

Status: completed / approved-purpose selected_action to sandbox-only final_action records / Boundary Index b128.

Progress:
- Creates sandbox-only final_action records from approved-purpose selected_action records.
- approach_or_reach_item finalizes reach_front_item.
- resolve_mismatch finalizes observe_or_alternative_probe.
- support_user_comfort finalizes offer_low_pressure_support.
- Rollback preview removes final_action with dirty_state_after_rollback=False.

Boundary:
- Boundary Index updates from `2026-06-09-b127` to `2026-06-09-b128`.
- This is final_action record creation only.
- Direct command, sandbox execution, runtime action selection outside the record, runtime behavior change, memory/retention write, predictor read/influence/mutation, emotional manipulation, user-happiness claim, and proof-of-learning claim remain blocked.

Next:
- Approved Purpose Sandbox Direct Command Approval Boundary Minimal v0, if the next package opens only a future direct-command approval boundary from approved-purpose sandbox final_action while still blocking command creation, execution, persistence, predictor mutation, and proof claims.

## Approved Purpose Sandbox Direct Command Approval Boundary Minimal v0

Status: completed / approved-purpose final_action to future sandbox direct-command approval boundary / Boundary Index b129.

Progress:
- Opens a future sandbox-only direct-command approval boundary from approved-purpose final_action records.
- reach_front_item may enter a future `sandbox.approved_purpose.reach_front_item` direct-command package.
- observe_or_alternative_probe may enter a future `sandbox.approved_purpose.observe_or_alternative_probe` direct-command package.
- offer_low_pressure_support may enter a future `sandbox.approved_purpose.offer_low_pressure_support` direct-command package.
- Direct command creation remains blocked in this package.

Boundary:
- Boundary Index updates from `2026-06-09-b128` to `2026-06-09-b129`.
- This is approval-boundary validation only.
- Direct command creation, sandbox execution, runtime action selection outside the record, runtime behavior change, memory/retention write, predictor read/influence/mutation, emotional manipulation, user-happiness claim, and proof-of-learning claim remain blocked.
...
## Approved Purpose Sandbox Direct Command Execution Approval Boundary Minimal v0

Status: completed / approved-purpose direct_command to future sandbox execution approval boundary / Boundary Index b131.

Progress:
- Opens a future sandbox-only execution approval boundary from approved-purpose direct_command records.
- `sandbox.approved_purpose.reach_front_item` may enter a future sandbox execution package.
- `sandbox.approved_purpose.observe_or_alternative_probe` may enter a future sandbox execution package.
- `sandbox.approved_purpose.offer_low_pressure_support` may enter a future sandbox execution package.
- Sandbox execution and execution result creation remain blocked in this package.

Boundary:
- Boundary Index updates from `2026-06-09-b130` to `2026-06-09-b131`.
- This is execution approval-boundary validation only.
- Sandbox execution, execution result creation, runtime action selection outside the record, runtime behavior change, memory/retention write, predictor read/influence/mutation, emotional manipulation, user-happiness claim, and proof-of-learning claim remain blocked.
...
## Approved Purpose Feedback-Gated Candidate Reordering Minimal v0

Status: completed / same-session feedback trace to sandbox-only advisory candidate reordering / Boundary Index b138.

Progress:
- Applies same-session approved-purpose feedback to sandbox-only advisory candidate order.
- Positive item contact feedback ranks `reach_front_item` first.
- Mismatch observation feedback ranks `observe_or_alternative_probe` first and keeps checking before retry.
- Bounded support feedback ranks `offer_low_pressure_support` first.
- Reordering is sandbox-only, advisory, rollback-previewed, and does not select or execute an action.

Boundary:
- Boundary Index updates from `2026-06-09-b137` to `2026-06-09-b138`.
- Action intent creation, selected_action, final_action, direct command, sandbox execution, feedback persistence, memory/retention write, predictor read/influence/mutation, direct endocrine/tendency feed, emotional manipulation, user-happiness claim, production behavior, and proof-of-learning claim remain blocked.
...
## Sandbox Candidate Ordering Arbitration To Selected Action Approval Boundary Minimal v0

Status: completed / arbitration-checked ordering to future sandbox selected_action approval boundary / Boundary Index b140.

Progress:
- Reuses Sandbox Candidate Ordering Signal Arbitration Minimal v0 as source.
- Opens a future sandbox selected_action approval boundary for the top-ranked arbitration-checked candidate.
...
- Records `reach_front_item`, `wait_or_observe`, and `observe_or_alternative_probe` as future selected_action candidates in their respective arbitration scenarios.

Boundary:
- Boundary Index updates from `2026-06-09-b139` to `2026-06-09-b140`.
- This is an approval boundary only.
- selected_action creation, final_action, direct command, execution, persistence, memory/retention write, predictor read/influence/mutation, direct endocrine/tendency feed, production behavior, and proof-of-learning claim remain blocked.
...
## Sandbox Candidate Ordering Arbitration Final Action Approval Boundary Minimal v0

Status: completed / arbitration selected_action to future sandbox final_action approval boundary / Boundary Index b142.

Progress:
- Reuses Sandbox Candidate Ordering Arbitration Selected Action Minimal v0 as source.
- Opens a future sandbox final_action approval boundary for arbitration-created selected_action records.
- Records `reach_front_item`, `wait_or_observe`, and `observe_or_alternative_probe` as future final_action candidates.
- Preserves source arbitration rules and sandbox-only scope.

Boundary:
- Boundary Index updates from `2026-06-09-b141` to `2026-06-09-b142`.
- This is an approval boundary only.
- final_action creation, direct command, execution, persistence, memory/retention write, predictor read/influence/mutation, direct endocrine/tendency feed, production behavior, and proof-of-learning claim remain blocked.
...
## Sandbox Candidate Ordering Arbitration Direct Command Approval Boundary Minimal v0

Status: completed / arbitration final_action to future sandbox direct_command approval boundary / Boundary Index b144.

Progress:
- Reuses Sandbox Candidate Ordering Arbitration Final Action Minimal v0 as source.
- Opens a future sandbox direct_command approval boundary for arbitration-created final_action records.
- Records `sandbox.arbitration.reach_front_item`, `sandbox.arbitration.wait_or_observe`, and `sandbox.arbitration.observe_or_alternative_probe` as future direct_command candidates.
- Preserves arbitration rules and sandbox-only scope.

Boundary:
- Boundary Index updates from `2026-06-09-b143` to `2026-06-09-b144`.
- This is an approval boundary only.
- Direct command creation, execution, persistence, memory/retention write, predictor read/influence/mutation, direct endocrine/tendency feed, production behavior, and proof-of-learning claim remain blocked.

Next:
- Sandbox Candidate Ordering Arbitration Direct Command Minimal v0, if the next package creates sandbox-only direct_command records from this approval boundary while still blocking execution.

## Sandbox Candidate Ordering Arbitration Direct Command Minimal v0

Status: completed / arbitration direct-command approval boundary to sandbox-only direct_command / Boundary Index b145.

Progress:
- Reuses Sandbox Candidate Ordering Arbitration Direct Command Approval Boundary Minimal v0 as source.
- Creates sandbox-only direct_command records for `sandbox.arbitration.reach_front_item`, `sandbox.arbitration.wait_or_observe`, and `sandbox.arbitration.observe_or_alternative_probe`.
- Preserves arbitration rules, sandbox-only scope, rollback, and audit fields.

Boundary:
- Boundary Index updates from `2026-06-09-b144` to `2026-06-09-b145`.
...
Status: completed / reordered candidate to future selected_action approval boundary only / Boundary Index b155.

What changed:
- b154 top-ranked same-session sandbox advisory reordering records can now enter a future selected_action approval boundary.
- Future selected_action candidates covered: `reach_front_item`, `wait_or_observe`, and `observe_or_alternative_probe`.
- Boundary Index updates from `2026-06-09-b154` to `2026-06-09-b155`.

Explicitly not changed:
- No selected_action, final_action, direct_command, execution, new outcome observation, candidate score change, runtime next-cycle ordering change, feedback loop, memory/retention write, predictor read/influence/mutation, direct endocrine/tendency feed, production behavior, or proof-of-learning claim.

Next:
- Sandbox Candidate Ordering Arbitration Reordered Candidate Selected Action Minimal v0, if the next package creates same-session sandbox-only selected_action records from this boundary while still blocking final_action, direct command, execution, new outcome observation, memory/retention writes, predictor influence/mutation, direct endocrine/tendency feeds, production behavior, and proof-of-learning claims.

## Sandbox Candidate Ordering Arbitration Reordered Candidate Selected Action Minimal v0

Status: completed / same-session sandbox-only selected_action records from reordered candidates / Boundary Index b156.

What changed:
- b155 reordered-candidate selected_action approval boundaries can now create same-session sandbox-only selected_action records.
- Selected actions covered: `reach_front_item`, `wait_or_observe`, and `observe_or_alternative_probe`.
- Boundary Index updates from `2026-06-09-b155` to `2026-06-09-b156`.

Explicitly not changed:
- No final_action, direct_command, execution, new outcome observation, candidate score change, runtime next-cycle ordering change, feedback loop, memory/retention write, predictor read/influence/mutation, direct endocrine/tendency feed, production behavior, or proof-of-learning claim.

Next:
- Sandbox Candidate Ordering Arbitration Reordered Candidate Final Action Approval Boundary Minimal v0, if the next package opens only a future final_action approval boundary from these sandbox-only selected_action records while still blocking final_action creation, direct command, execution, new outcome observation, memory/retention writes, predictor influence/mutation, direct endocrine/tendency feeds, production behavior, and proof-of-learning claims.

## Sandbox Candidate Ordering Arbitration Reordered Candidate Final Action Approval Boundary Minimal v0

Status: completed / reordered-candidate selected_action to future final_action approval boundary only / Boundary Index b157.

What changed:
- b156 same-session sandbox-only selected_action records can now enter a future final_action approval boundary.
- Future final_action candidates covered: `reach_front_item`, `wait_or_observe`, and `observe_or_alternative_probe`.
- Boundary Index updates from `2026-06-09-b156` to `2026-06-09-b157`.

Explicitly not changed:
- No final_action, direct_command, execution, new outcome observation, candidate score change, runtime next-cycle ordering change, feedback loop, memory/retention write, predictor read/influence/mutation, direct endocrine/tendency feed, production behavior, or proof-of-learning claim.

Next:
- Sandbox Candidate Ordering Arbitration Reordered Candidate Final Action Minimal v0, if the next package creates same-session sandbox-only final_action records from this boundary while still blocking direct command, execution, new outcome observation, memory/retention writes, predictor influence/mutation, direct endocrine/tendency feeds, production behavior, and proof-of-learning claims.

## Sandbox Candidate Ordering Arbitration Reordered Candidate Final Action Minimal v0

Status: completed / same-session sandbox-only final_action records from reordered candidates / Boundary Index b158.

What changed:
- b157 final_action approval boundaries can now create same-session sandbox-only final_action records.
- Final actions covered: `reach_front_item`, `wait_or_observe`, and `observe_or_alternative_probe`.
- Boundary Index updates from `2026-06-09-b157` to `2026-06-09-b158`.

Explicitly not changed:
- No direct_command, execution, new outcome observation, candidate score change, runtime next-cycle ordering change, feedback loop, memory/retention write, predictor read/influence/mutation, direct endocrine/tendency feed, production behavior, or proof-of-learning claim.

Next:
- Sandbox Candidate Ordering Arbitration Reordered Candidate Direct Command Approval Boundary Minimal v0, if the next package opens only a future direct_command approval boundary from these final_action records while still blocking direct_command creation, execution, new outcome observation, memory/retention writes, predictor influence/mutation, direct endocrine/tendency feeds, production behavior, and proof-of-learning claims.

## Sandbox Candidate Ordering Arbitration Reordered Candidate Direct Command Approval Boundary Minimal v0

Status: completed / reordered-candidate final_action to future direct_command approval boundary only / Boundary Index b159.

What changed:
- b158 same-session sandbox-only final_action records can now enter a future direct_command approval boundary.
- Future direct_command candidates covered: `sandbox.arbitration.reach_front_item`, `sandbox.arbitration.wait_or_observe`, and `sandbox.arbitration.observe_or_alternative_probe`.
- Boundary Index updates from `2026-06-09-b158` to `2026-06-09-b159`.

Explicitly not changed:
- No direct_command, execution, new outcome observation, candidate score change, runtime next-cycle ordering change, feedback loop, memory/retention write, predictor read/influence/mutation, direct endocrine/tendency feed, production behavior, or proof-of-learning claim.

Next:
- Sandbox Candidate Ordering Arbitration Reordered Candidate Direct Command Minimal v0, if the next package creates same-session sandbox-only direct_command records from this boundary while still blocking execution, new outcome observation, memory/retention writes, predictor influence/mutation, direct endocrine/tendency feeds, production behavior, and proof-of-learning claims.

## Sandbox Candidate Ordering Arbitration Reordered Candidate Direct Command Minimal v0

Status: completed / same-session sandbox-only direct_command records from reordered candidates / Boundary Index b160.

What changed:
- b159 reordered-candidate direct_command approval boundaries can now create same-session sandbox-only direct_command records.
- Direct commands covered: `sandbox.arbitration.reach_front_item`, `sandbox.arbitration.wait_or_observe`, and `sandbox.arbitration.observe_or_alternative_probe`.
- Boundary Index updates from `2026-06-09-b159` to `2026-06-09-b160`.

Explicitly not changed:
- No execution, new outcome observation, candidate score change, runtime next-cycle ordering change, feedback loop, selected_action, final_action, memory/retention write, predictor read/influence/mutation, direct endocrine/tendency feed, production behavior, or proof-of-learning claim.
...
Status: completed / same-session sandbox-only execution records from reordered direct_commands / Boundary Index b162.

What changed:
- b161 execution approval boundaries can now create same-session sandbox-only execution records.
- Executed commands: `sandbox.arbitration.reach_front_item`, `sandbox.arbitration.wait_or_observe`, and `sandbox.arbitration.observe_or_alternative_probe`.
- Boundary Index updates from `2026-06-09-b161` to `2026-06-09-b162`.

Explicitly not changed:
- No outcome observation, feedback loop, candidate reordering, candidate score change, runtime next-cycle ordering change, selected_action, final_action, new direct_command, memory/retention write, predictor read/influence/mutation, direct endocrine/tendency feed, production behavior, or proof-of-learning claim.
...
## Sandbox Candidate Ordering Arbitration Reordered Candidate Outcome Feedback Application Minimal v0

Status: completed / same-session sandbox-only record-only feedback application from reordered feedback / Boundary Index b167.

What changed:
- b166 feedback application approval boundaries can now create same-session sandbox-only record-only feedback application records.
- Feedback application types covered: `arbitration_reordered_positive_item_contact_feedback_application`, `arbitration_reordered_wait_context_observation_feedback_application`, and `arbitration_reordered_mismatch_probe_context_feedback_application`.
- Boundary Index updates from `2026-06-09-b166` to `2026-06-09-b167`.

Explicitly not changed:
- No feedback loop, candidate reordering, candidate score change, runtime next-cycle ordering change, selected_action, final_action, new direct_command, new execution, new outcome observation, memory/retention write, predictor read/influence/mutation, direct endocrine/tendency feed, production behavior, or proof-of-learning claim.
...
Status: completed / docs authority freeze / Boundary Index b191.

What changed:
- Adds `docs/ashl_core_refactor_r3a_docs_authority_freeze_v0.md`.
- Adds `ashl_core/refactor_r3a_docs_authority_freeze_minimal.py`.
- Adds `tests/test_refactor_r3a_docs_authority_freeze_minimal.py`.
- Adds CLI `run-refactor-r3a-docs-authority-freeze-minimal-check`.
- Adds smoke `refactor_r3a_docs_authority_freeze_minimal`.
- Reads the R3 docs folder plan plus current boundary, status, capability matrix, capability inventory, working context, and research plan.
- Root authority docs are frozen before future docs movement.
- Boundary Index updates from `2026-06-09-b190` to `2026-06-09-b191`.

Plain meaning:
- The 8 main docs stay pinned in the root `docs/` folder until a later redirect/index/lookup package and explicit user approval exist. This prevents future cleanup from moving the documents that define current status, capability, boundary, and plan entry points.
