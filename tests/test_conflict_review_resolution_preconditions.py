...
class ConflictReviewResolutionPreconditionsTests(unittest.TestCase):
    def test_all_preconditions_met_for_matching_approved_review(self):
        ...
        for value in preconditions["preconditions"].values():
            self.assertTrue(value)
        self.assertTrue(preconditions["all_preconditions_met"])
        self.assertEqual(preconditions["failed_preconditions"], [])
        self.assertFalse(preconditions["resolution_activation_applied"])
        self.assertFalse(preconditions["conflict_changed"])
        self.assertFalse(preconditions["selection_changed"])
        self.assertFalse(preconditions["activation_changed"])

    def test_missing_stable_conflict_key_blocks_preconditions(self):
        trace = dict(conflict_trace())
        trace.pop("stable_conflict_key")
        preconditions = build_conflict_review_resolution_preconditions(trace, [], candidate_lesson_id="lesson_002")

        self.assertFalse(preconditions["preconditions"]["stable_conflict_key_exists"])
        self.assertFalse(preconditions["all_preconditions_met"])
        self.assertIn("stable_conflict_key_exists", preconditions["failed_preconditions"])
        self.assertFalse(preconditions["resolution_activation_applied"])

    def test_missing_matching_review_item_blocks_preconditions(self):
        trace = conflict_trace()
        preconditions = build_conflict_review_resolution_preconditions(trace, [], candidate_lesson_id="lesson_002")

        self.assertFalse(preconditions["preconditions"]["matching_review_item_exists"])
        self.assertFalse(preconditions["all_preconditions_met"])
        self.assertIn("matching_review_item_exists", preconditions["failed_preconditions"])
        self.assertFalse(preconditions["resolution_activation_applied"])

    def test_pending_unreviewed_blocks_preconditions(self):
        trace = conflict_trace()
        preconditions = build_conflict_review_resolution_preconditions(
            trace,
            [review_for(trace["stable_conflict_key"], "lesson_002", "pending")],
            candidate_lesson_id="lesson_002",
        )

        self.assertFalse(preconditions["preconditions"]["review_state_is_reviewed"])
        self.assertFalse(preconditions["preconditions"]["approval_state_is_approved"])
        self.assertFalse(preconditions["all_preconditions_met"])
        self.assertIn("review_state_is_reviewed", preconditions["failed_preconditions"])
        self.assertIn("approval_state_is_approved", preconditions["failed_preconditions"])
        self.assertFalse(preconditions["resolution_activation_applied"])

    def test_rejected_review_is_blocker_only(self):
        trace = conflict_trace()
        ...
        preconditions = build_conflict_review_resolution_preconditions(trace, [rejected], candidate_lesson_id="lesson_002")

        self.assertFalse(preconditions["preconditions"]["approval_state_is_approved"])
        self.assertFalse(preconditions["all_preconditions_met"])
        self.assertEqual(preconditions["blocked_reason"], "rejected_review_blocks_resolution")
        self.assertFalse(preconditions["resolution_activation_applied"])
        self.assertEqual(lessons, before)

    def test_conflicting_reviews_block_preconditions(self):
        trace = conflict_trace()
        ...
        preconditions = build_conflict_review_resolution_preconditions(trace, [approved, rejected], candidate_lesson_id="lesson_002")

        self.assertFalse(preconditions["preconditions"]["no_conflicting_review_for_same_conflict_candidate"])
        self.assertFalse(preconditions["all_preconditions_met"])
        self.assertIn("no_conflicting_review_for_same_conflict_candidate", preconditions["failed_preconditions"])
        self.assertEqual(preconditions["blocked_reason"], "blocked_by_conflicting_reviews")
        self.assertFalse(preconditions["resolution_activation_applied"])
        self.assertTrue(trace["review_required"])

    def test_multiple_approved_candidates_block_preconditions(self):
        trace = conflict_trace()
        ...
        preconditions = build_conflict_review_resolution_preconditions(trace, [approved_a, approved_b], candidate_lesson_id="lesson_002")

        self.assertFalse(preconditions["preconditions"]["exactly_one_approved_candidate"])
        self.assertFalse(preconditions["all_preconditions_met"])
        self.assertIn("exactly_one_approved_candidate", preconditions["failed_preconditions"])
        self.assertEqual(preconditions["blocked_reason"], "blocked_by_multiple_approvals")
        self.assertFalse(preconditions["resolution_activation_applied"])
    ...
    def test_runtime_conflict_id_cannot_be_only_matching_anchor(self):
        ...
        preconditions = build_conflict_review_resolution_preconditions(trace, [runtime_only], candidate_lesson_id="lesson_002")

        self.assertFalse(preconditions["preconditions"]["target_id_matches_stable_conflict_key"])
        self.assertFalse(preconditions["all_preconditions_met"])
        self.assertNotEqual(runtime_only["target_id"], trace["stable_conflict_key"])

    def test_preconditions_do_not_introduce_conflict_side_effect(self):
        trace = conflict_trace()
    ...
    def test_preconditions_do_not_introduce_selection_side_effect(self):
    ...
    def test_preconditions_do_not_affect_strict_supersede_activation(self):
        old = mark_lesson_stale(lesson_east())
        old["stale_reason"] = "manual: precondition fixture"
    ...
    def test_preconditions_do_not_modify_review_item_metadata(self):
        trace = conflict_trace()
        approved = review_for(trace["stable_conflict_key"], "lesson_002", "approved", "review_approved")
        rejected = review_for(trace["stable_conflict_key"], "lesson_002", "rejected", "review_rejected")
        ...
        build_conflict_review_resolution_preconditions(trace, reviews, candidate_lesson_id="lesson_002")
    ...
    def test_preconditions_do_not_modify_lesson_lifecycle_metadata(self):
        trace = conflict_trace()
        lessons = [lesson_east(), lesson_west()]
        before = copy.deepcopy(lessons)
