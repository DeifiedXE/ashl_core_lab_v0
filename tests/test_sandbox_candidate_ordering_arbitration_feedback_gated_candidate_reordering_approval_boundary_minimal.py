...
class SandboxCandidateOrderingArbitrationFeedbackGatedCandidateReorderingApprovalBoundaryMinimalTests(
    unittest.TestCase
):
    @classmethod
    def setUpClass(cls):
        cls.sources = run_sandbox_candidate_ordering_arbitration_outcome_feedback_application_minimal_check()[
            "valid_records"
        ]
        cls.result = (
            run_sandbox_candidate_ordering_arbitration_feedback_gated_candidate_reordering_approval_boundary_minimal_check()
    ...
    def test_valid_reordering_approval_boundaries_are_created(self):
        for record in self.records:
            result = validate_sandbox_candidate_ordering_arbitration_feedback_gated_candidate_reordering_approval_boundary_record(
                record
            )
            ...
            self.assertEqual(
                record["record_type"],
                "sandbox_candidate_ordering_arbitration_feedback_gated_candidate_reordering_approval_boundary_minimal",
            )
            self.assertEqual(record["boundary_index_after"], BOUNDARY_INDEX_AFTER)
            self.assertTrue(
                record["feedback_gated_reordering_approval_boundary"][
                    "future_candidate_reordering_allowed"
                ]
            )

    def test_reach_front_item_application_can_enter_future_reordering(self):
        record = build_sandbox_candidate_ordering_arbitration_feedback_gated_candidate_reordering_approval_boundary_record(
            self.sources[0]
        )
